...
# Step 7: Build JupyterLab assets (build process might take some time)
RUN jupyter lab build

# Step 8: Working Directory
WORKDIR $HOME

# Step 9: Set environment variables for user configuration
ARG NB_USER=jovyan
ARG NB_UID=1000
ENV USER ${NB_USER}
ENV NB_UID ${NB_UID}
...
# Step 10: Set user permissions and working directory
WORKDIR ${HOME}

USER root

# Step 11: Install curl and other apt-get dependencies
RUN apt-get update && apt-get install -y curl libicu66

# Step 12: Install .NET Core SDK and dependencies (if necessary for your repo)
RUN apt-get update \
...
# Step 13: Install .NET Core SDK (if needed)
...
# Step 15: Copy package sources
COPY ./NuGet.config ${HOME}/nuget.config

# Step 16: Ensure ownership of files for the user
RUN chown -R ${NB_UID} ${HOME}

# Step 17: Set the user back to the non-root user
USER ${USER}
